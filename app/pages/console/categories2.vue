    ...
    >
      ...
      <div class="p-4 border-b border-neutral-200/80 dark:border-neutral-800 flex items-center justify-between gap-3 bg-neutral-50/50 dark:bg-neutral-900/50">
        <UInput
          ...
          placeholder="Filter categories or tags..."
      ...
      <div class="flex-1 overflow-auto">
        ...
        >
          ...
          <template #active-cell="{ row }">
            <UBadge
              :label="generateStatus(row.original.active).label"
          ...
          <template #tags-cell="{ row }">
            <div class="flex flex-wrap gap-1 max-w-55">
              <UBadge
                v-for="(tag, idx) in (row.original.tags || []).slice(0, 3)"
                :key="idx"
                :label="tag"
                color="neutral"
                variant="outline"
              ...
              />
              <UBadge
                v-if="(row.original.tags || []).length > 3"
                :label="`+${row.original.tags?.length ?? 0 - 3}`"
                color="neutral"
                variant="soft"
                size="xs"
              />
              <span v-if="!(row.original.tags && row.original.tags.length)" class="text-xs text-neutral-400 font-normal">
                No tags
              </span>
            </div>
          </template>
          ...
          <template #products-cell="{ row }">
            <UBadge
              :label="`${row.original.products?.length || 0} items`"
              color="neutral"
              variant="soft"
              size="sm"
            />
    ...
    >
      <template #header>
        <div class="flex items-center gap-2">
          <UIcon
            :name="state.current.publicId ? 'i-lucide-file-edit' : 'i-lucide-folder-plus'"
        ...
        <UBadge
          v-if="state.current.publicId"
          :label="state.current.publicId"
          color="neutral"
          variant="outline"
          size="xs"
        />
      ...
      <UFormField label="Category Name" required>
        <UInput
          v-model="state.current.name"
          placeholder="e.g. Architectural Models"
          class="w-full"
        />
      </UFormField>
      ...
      <UFormField
        label="Tags"
        description="Type tag name and press Enter or comma to add."
      >
        <div class="space-y-2 w-full">
          ...
          <div class="flex flex-wrap gap-1.5 min-h-8 p-2 bg-neutral-50 dark:bg-neutral-950/50 rounded-lg border border-neutral-200/60 dark:border-neutral-800">
            <UBadge
              v-for="(tag, index) in state.current.tags"
              :key="index"
              color="primary"
              variant="soft"
              class="gap-1"
            >
              ...
              <UIcon
                name="i-lucide-x"
                class="size-3 cursor-pointer hover:text-red-500 transition-colors"
                @click="removeTag(index)"
              />
            </UBadge>
            <span v-if="!state.current.tags.length" class="text-xs text-neutral-400 py-0.5">
              No tags attached
            </span>
          </div>
        </div>
