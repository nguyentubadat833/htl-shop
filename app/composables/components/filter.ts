...
const _useFilter = () => {
  const openToogleStatus = ref(false);
  const filterStatus = ref(false);
  const filterCategories = useState(() => ref<CategoryFilter[]>([]));
  const filterState = useState(() =>
    reactive<ProductListFilterState>({
      keyWork: undefined,
  ...
  const filterTags = computed<string[]>(() => {
    return [...filterState.value.plans, ...filterState.value.categoryTypes, ...filterCategories.value.map((ftg) => ftg.name)];
  });
